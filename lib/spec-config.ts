...
export const SPEC_CONFIGS: Record<string, SpecConfig> = {
  ...
  laptop: {
    ...
    columns: [
      ...
      { key: 'ram_size',          label: 'RAM',     unit: 'GB', format: 'number', parse_from_title: /(\d+)\s*gb\s*(ram|memory)/i,      priority: 'high'   },
      { key: 'display_brightness',label: 'Parlaklık',unit:'nits',format: 'number', parse_from_title: /(\d+)\s*nits?/i,                  priority: 'high'   },
      { key: 'storage',           label: 'Depolama',unit: 'GB', format: 'number', parse_from_title: /(\d+)\s*(gb|tb)\s*(ssd|nvme)/i,   priority: 'medium' },
