...
export function TechSpecsTable({ productA, productB, categorySlug, className = '' }: TechSpecsTableProps) {
  ...
  const formatValue = (value: string | number | null, format?: string, unit?: string): string => {
    ...
    if (format === 'number'     && typeof value === 'number') return `${value.toLocaleString('tr-TR')}${unit ? ` ${unit}` : ''}`;
    if (format === 'percentage' && typeof value === 'number') return `${value}%`;
    if (format === 'currency'   && typeof value === 'number') return `₺${value.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}`;
  ...
  const winnerBgB   = `${GOLD}14`;
  const winnerBordB = `${GOLD}40`;
    ...
    <div className={`w-full ${className}`}>
      ...
      <div className="block lg:hidden space-y-4">
        {displayColumns.map((column) => {
          ...
          return (
            <div key={column.key} className="backdrop-blur-xl bg-gray-900/40 rounded-xl border border-gray-800/50 p-4">
              <div className="text-sm font-semibold text-gray-400 mb-3">{column.label}</div>
              <div className="space-y-2">
                {/* Ürün A */}
                <div className={`flex items-center justify-between p-3 rounded-lg ${
                  comparison.winner === 'a' ? 'bg-emerald-500/10 border border-emerald-500/30' : 'bg-gray-800/30'
                }`}>
                  <span className="text-xs text-gray-500">{shortName(productA)}</span>
                  <span className={`text-lg font-bold ${comparison.winner === 'a' ? 'text-emerald-400' : 'text-gray-300'}`}>
                ...
                <div
                  ...
                  style={comparison.winner === 'b' ? { background: winnerBgB, border: `1px solid ${winnerBordB}` } : { background: 'rgba(31,41,55,0.3)' }}
                >
                  <span className="text-xs text-gray-500">{shortName(productB)}</span>
                  <span
                    className="text-lg font-bold"
                    style={{ color: comparison.winner === 'b' ? GOLD_BRIGHT : '#D1D5DB' }}
                  >
        ...
        <div className="backdrop-blur-xl bg-gray-900/40 rounded-xl border border-gray-800/50 p-4">
          ...
          <div className="space-y-2">
            <div className={`flex items-center justify-between p-3 rounded-lg ${priceA < priceB ? 'bg-emerald-500/10 border border-emerald-500/30' : 'bg-gray-800/30'}`}>
              <span className="text-xs text-gray-500">{shortName(productA)}</span>
              <span className={`text-lg font-bold ${priceA < priceB ? 'text-emerald-400' : 'text-gray-300'}`}>
                ₺{priceA.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}
              </span>
            </div>
            <div
              className="flex items-center justify-between p-3 rounded-lg"
              style={priceB < priceA ? { background: winnerBgB, border: `1px solid ${winnerBordB}` } : { background: 'rgba(31,41,55,0.3)' }}
            >
              <span className="text-xs text-gray-500">{shortName(productB)}</span>
              <span className="text-lg font-bold" style={{ color: priceB < priceA ? GOLD_BRIGHT : '#D1D5DB' }}>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-800/50">
              ...
              <th className="text-center p-4 text-sm font-semibold w-1/3" style={{ color: GOLD_BRIGHT }}>
          ...
          <tbody>
            {displayColumns.map((column, index) => {
              ...
              return (
                <tr
                  key={column.key}
                  className={`border-b border-gray-800/30 transition-colors hover:bg-gray-800/20 ${index % 2 === 0 ? 'bg-gray-900/20' : ''}`}
                >
                  {/* Özellik ismi */}
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                      <div
                        ...
                        style={{ background: column.priority === 'high' ? GOLD : '#4B5563' }}
                      />
                  ...
                  {/* Ürün A */}
                  <td className={`p-4 text-center ${comparison.winner === 'a' ? 'bg-emerald-500/5' : ''}`}>
                  ...
                  <td
                    ...
                    style={comparison.winner === 'b' ? { background: `${GOLD}0D` } : {}}
                  >
                    <div className="flex items-center justify-center gap-2">
                      {comparison.winner === 'b' && (
                        <svg className="w-5 h-5" style={{ color: GOLD_BRIGHT }} fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                        </svg>
                      )}
                      <span
            ...
            <tr className="border-b border-gray-800/30 hover:bg-gray-800/20">
              ...
              <td className={`p-4 text-center ${priceA < priceB ? 'bg-emerald-500/5' : ''}`}>
                <span className={`text-lg font-bold ${priceA < priceB ? 'text-emerald-400' : 'text-gray-300'}`}>
                  ₺{priceA.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}
                </span>
              </td>
              <td
                className="p-4 text-center"
                style={priceB < priceA ? { background: `${GOLD}0D` } : {}}
              >
                <span className="text-lg font-bold" style={{ color: priceB < priceA ? GOLD_BRIGHT : '#D1D5DB' }}>
                  ₺{priceB.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}
                </span>
