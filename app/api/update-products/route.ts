...
function parsePrice(raw: any): number | null {
  ...
  if (typeof raw === 'object' && raw !== null && 'value' in raw) {
    ...
    if (dotIdx !== -1 && str.length - dotIdx - 1 === 3) return Math.round(num * 1000);
    return num;
  }
  if (typeof raw === 'number') {
    ...
    const dotIdx = str.indexOf('.');
    if (dotIdx !== -1 && str.length - dotIdx - 1 === 3) return Math.round(raw * 1000);
    return raw;
  }
  if (typeof raw === 'string') {
    ...
    if (parts.length > 1 && parts[parts.length - 1].length === 3) return parseInt(clean.replace(/\./g, ''), 10) || null;
...
function cleanName(title: string): string {
  if (!title) return '';
  const cleaned = title.split(/,|\s*\|\s*|\s+-\s+(?=[A-Z])/)[0].trim();
  return cleaned.length > 60 ? cleaned.substring(0, 57) + '...' : cleaned;
}

function extractBrand(title: string, apifyBrand?: string): string {
  if (apifyBrand && apifyBrand.length > 1 && !apifyBrand.startsWith('Marka:')) return apifyBrand;
  const knownBrands = [
...
function processPhone(item: ApifyProduct, stars: number, reviewCount: number) {
  ...
  return {
    specs: { ram_gb: ram, storage_gb: storage, screen_inch: screen, camera_mp: mp, battery_mah: mAh, has_5g: t.includes('5g'), has_nfc: d.includes('nfc') },
    scores: { camera_score, performance_score, battery_score, display_score, overall_score },
    spec_labels: { 'RAM': ram ? `${ram} GB` : null, 'Depolama': storage ? `${storage} GB` : null, 'Ekran': screen ? `${screen} inç` : null, 'Kamera': mp ? `${mp} MP` : null, 'Batarya': mAh ? `${mAh} mAh` : null, '5G': t.includes('5g') ? 'Evet' : 'Hayır' },
  };
}
...
function processLaptop(item: ApifyProduct, stars: number, reviewCount: number) {
  ...
  if (ssdMatch) { ssd = parseInt(ssdMatch[1]); if (ssdMatch[0].toLowerCase().includes('tb')) ssd *= 1000; }
  const screen = screenMatch ? parseFloat(screenMatch[1].replace(',', '.')) : null;
  ...
  const overall_score = Math.round(performance_score * 0.35 + ram_score * 0.25 + display_score * 0.25 + build_quality * 0.15);
  return {
    ...
    spec_labels: { 'RAM': ram ? `${ram} GB` : null, 'SSD': ssd ? (ssd >= 1000 ? `${ssd/1000} TB` : `${ssd} GB`) : null, 'Ekran': screen ? `${screen} inç` : null, 'Yenileme Hızı': hz ? `${hz} Hz` : null },
  };
}
...
function processRobotVacuum(item: ApifyProduct, stars: number, reviewCount: number) {
  ...
  const overall_score = Math.round(suction_score * 0.35 + navigation_score * 0.30 + battery_score * 0.20 + noise_score * 0.15);
  return {
    ...
    spec_labels: { 'Emiş Gücü': pa ? `${pa} Pa` : null, 'Batarya': mAh ? `${mAh} mAh` : null, 'Gürültü': db ? `${db} dB` : null, 'Çalışma Süresi': mins ? `${mins} dk` : null, 'Navigasyon': hasLidar ? 'LiDAR' : d.includes('laser') ? 'Lazer' : 'Standart', 'Paspas': hasMop ? 'Var' : 'Yok' },
  };
}
...
function processHeadphone(item: ApifyProduct, stars: number, reviewCount: number) {
  ...
  const overall_score = Math.round(sound_quality * 0.35 + noise_cancelling * 0.25 + battery_score * 0.20 + comfort_score * 0.20);
  return {
    ...
    spec_labels: { 'Pil Ömrü': hours ? `${hours} saat` : (!isWireless ? 'Kablolu' : null), 'ANC': hasANC ? 'Var' : 'Yok', 'Bağlantı': isWireless ? 'Kablosuz' : 'Kablolu', 'Hi-Res': hasHiRes ? 'Evet' : 'Hayır' },
  };
}
...
function processSmartwatch(item: ApifyProduct, stars: number, reviewCount: number) {
  ...
  const overall_score = Math.round(fitness_score * 0.30 + battery_score * 0.25 + display_score * 0.25 + health_features * 0.20);
  return {
    ...
    spec_labels: { 'Pil Ömrü': days ? `${days} gün` : null, 'Kasa Boyutu': size ? `${size} mm` : null, 'GPS': hasGPS ? 'Var' : 'Yok', 'ECG': hasECG ? 'Var' : 'Yok', 'SpO2': hasSpO2 ? 'Var' : 'Yok', 'Ekran': hasAMOLED ? 'AMOLED' : 'LCD' },
  };
}
...
function processTablet(item: ApifyProduct, stars: number, reviewCount: number) {
  ...
  const overall_score = Math.round(performance_score * 0.30 + display_score * 0.30 + battery_score * 0.20 + ram_score * 0.20);
  return {
    ...
    spec_labels: { 'RAM': ram ? `${ram} GB` : null, 'Depolama': storage ? `${storage} GB` : null, 'Ekran': screen ? `${screen} inç` : null, 'Batarya': mAh ? `${mAh} mAh` : null },
  };
}
...
function processTV(item: ApifyProduct, stars: number, reviewCount: number) {
  ...
  const overall_score = Math.round(picture_quality * 0.35 + screen_size_score * 0.25 + smart_score * 0.20 + sound_score * 0.10 + hdr_score * 0.10);
  return {
    ...
    spec_labels: { 'Ekran Boyutu': size ? `${size} inç` : null, 'Çözünürlük': is8K ? '8K' : is4K ? '4K UHD' : 'Full HD', 'Panel': isOLED ? 'OLED' : isQLED ? 'QLED' : 'LED', 'Yenileme Hızı': hz ? `${hz} Hz` : null, 'HDR': hasDolby ? 'Dolby Vision' : is4K ? 'HDR10' : 'Yok', 'Smart TV': smartOS ? 'Evet' : 'Hayır' },
  };
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNyeXB1bGZ4YmNraGVya21yamdzIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MTE1NzMwNywiZXhwIjoyMDg2NzMzMzA3fQ.2k9r-KEWjTXmQjTkFm1wMztoquQGVhz2aiUD1R_UJz4';
    ...
    if (rawProducts.length === 0) return NextResponse.json({ success: true, message: 'Ürün bulunamadı', inserted: 0 });

    const { data: category, error: catErr } = await supabase
      .from('categories').select('id').eq('slug', categorySlug).single();
    if (catErr || !category) return NextResponse.json({ error: `Kategori bulunamadı: ${categorySlug}` }, { status: 500 });
    ...
    const allProducts: SupabaseProduct[] = rawProducts
      ...
      .map((item) => {
        ...
        switch (categorySlug) {
          ...
          default:
            processed = { specs: {}, scores: { overall_score: Math.min(10, Math.round(stars * 2)) }, spec_labels: {} };
        }
        ...
        return {
          ...
          specifications: { stars, reviewsCount: reviewCount, ...processed.specs, ...processed.scores, spec_labels: processed.spec_labels },
          is_active:      true,
    ...
    for (let i = 0; i < allProducts.length; i += BATCH) {
      ...
      if (error) { console.error(`Batch ${i} upsert hatası:`, error.message); errorCount += batch.length; }
      else { upsertedCount += batch.length; }
    }

    console.log(`🚫 ${filteredOut} ürün filtre ile reddedildi`);
    console.log(`💾 ${upsertedCount} ürün upsert edildi, ${errorCount} hata`);

    return NextResponse.json({ success: true, upserted: upsertedCount, errors: errorCount, filtered: filteredOut, category: categorySlug, total: rawProducts.length });
