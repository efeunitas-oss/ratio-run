...
// [İbrahim Çimen - Executive Summary Edition]

import React, { useState, useMemo } from 'react';
import { Vehicle, ComparisonResult, VehicleAnalysis, RobotVacuum } from './types';
...
export default function RatioRunApp() {
  // --- KATEGORİ VE VERİ YÖNETİMİ ---
  const [category, setCategory] = useState<'CAR' | 'VACUUM'>('CAR');
  
  // Kategoriye göre aktif veri setini belirle
  const currentData = category === 'CAR' ? SAMPLE_VEHICLES : (SAMPLE_VACUUMS as any);

  const [selectedVehicle1, setSelectedVehicle1] = useState<string>(SAMPLE_VEHICLES[0].id);
  const [selectedVehicle2, setSelectedVehicle2] = useState<string>(SAMPLE_VEHICLES[1].id);
  const [showDetails, setShowDetails] = useState<boolean>(true);

  // Kategori değiştiğinde seçimleri resetle
  const handleCategoryChange = (newCat: 'CAR' | 'VACUUM') => {
    setCategory(newCat);
    const newData = newCat === 'CAR' ? SAMPLE_VEHICLES : (SAMPLE_VACUUMS as any);
    setSelectedVehicle1(newData[0].id);
    setSelectedVehicle2(newData[1].id);
  };
  
  // --- KARAR MOTORU (BRAIN) ---
  const comparison = useMemo<any>(() => {
    const p1 = currentData.find((v: any) => v.id === selectedVehicle1);
    const p2 = currentData.find((v: any) => v.id === selectedVehicle2);
    
    if (!p1 || !p2) return null;
    
    // Kategoriye göre doğru algoritmayı çağır
    if (category === 'CAR') {
      return compareVehicles(p1 as Vehicle, p2 as Vehicle);
    } else {
      return compareVacuums(p1 as RobotVacuum, p2 as RobotVacuum);
    }
  }, [selectedVehicle1, selectedVehicle2, category, currentData]);
  
  if (!comparison) return null;
  
  const isVehicle = category === 'CAR';
  const winnerAnalysis = isVehicle 
  ...
  return (
    <div className="min-h-screen bg-black text-gray-100 selection:bg-amber-500/30">
      ...
      {/* Verdict Section - The Money Shot */}
      <section className="relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 py-12 relative z-10">
          {isVehicle && comparison.segmentWarning && (
            ...
            <div className="flex flex-col md:flex-row items-start justify-between mb-6 gap-8">
              ...
              <div className="md:ml-6 text-right bg-white/5 p-6 rounded-2xl border border-white/5 min-w-[200px]">
                ...
                <div className="text-6xl font-black text-amber-500 italic tracking-tighter">
                  {isVehicle ? winnerAnalysis!.finalScore.toFixed(1) : comparison.score1}
                </div>
                <div className="text-xs text-zinc-400 mt-2 font-bold uppercase tracking-widest">
                  {isVehicle ? getScoreLabel(winnerAnalysis!.finalScore) : 'Analiz Tamamlandı'}
                </div>
              </div>
            </div>
      ...
      {/* Score Comparison Cards - Orijinal Detaylı Yapı */}
      {isVehicle && (
      ...
      {/* Detailed Comparison Table - 670 Satırlık Emeğin Olduğu Yer */}
      {isVehicle && (
...
// ============================================================================
// ORIGINAL SUB-COMPONENTS (670 LINE CORE)
// ============================================================================
...
// 670 SATIRLIK ASIL TABLO MİMARİSİ
      ...
      <table className="w-full text-sm">
        <thead className="bg-zinc-900 border-b border-zinc-800">
          <tr className="text-center">
            ...
            <th className="p-6 text-white font-black text-lg tracking-tighter">{v1.name}</th>
            <th className="p-6 text-white font-black text-lg tracking-tighter">{v2.name}</th>
